...
public class ContentListElementResourceTest
	...
	protected ContentListElement
		...
		else if (contentType.equals("WikiPage")) {
			return _addWikiPage(
				testGroup.getGroupId(), contentListId,
	...
	private GenericContentListElement _addWikiPage(
			Long contentSpaceId, Long assetListEntryId,
			GenericContentListElement genericContentListElement)
		throws Exception {
		...
		WikiNode wikiNode = nodes.get(0);

		WikiPage wikiPage = WikiPageLocalServiceUtil.addPage(
			TestPropsValues.getUserId(), wikiNode.getNodeId(),
			genericContentListElement.getTitle(), RandomTestUtil.randomString(),
			RandomTestUtil.randomString(), false, serviceContext);

		AssetEntry assetEntry = AssetEntryLocalServiceUtil.fetchEntry(
			contentSpaceId, wikiPage.getUuid());
