...
public class FileURLReference implements Serializable {
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "The Base64-encoded file content. On write, used to create a new file when `externalReferenceCode` does not resolve in the destination. On read, returned only when requested via `nestedFields=fileBase64`."
	)
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		description = "A URL the file can be fetched from. On read, the canonical URL of the stored file. On write, used to download bytes (supports `http://`, `https://` and `lar://`) when `externalReferenceCode` does not resolve and `fileBase64` is absent."
	)
