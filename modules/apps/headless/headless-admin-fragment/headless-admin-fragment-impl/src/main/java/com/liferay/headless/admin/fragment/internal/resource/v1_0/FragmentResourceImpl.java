...
import com.liferay.fragment.exception.FragmentEntryVersionsRequiredException;
...
public class FragmentResourceImpl extends BaseFragmentResourceImpl {
	...
	public Fragment putSiteFragment(
		...
		try {
			...
			return _updateFragmentEntry(fragmentEntry, groupId, fragment);
		}
		catch (NoSuchModelException noSuchModelException) {
			if (_log.isDebugEnabled()) {
				_log.debug(noSuchModelException);
			...
			if (draftFragmentEntry != null) {
				return _updateFragmentEntry(
					draftFragmentEntry, groupId, fragment);
			}
	...
	private void _updateDraft(
			long fragmentEntryId, FragmentVersion draftFragmentVersion)
		throws Exception {
	...
	private Fragment _updateFragmentEntry(
			FragmentEntry fragmentEntry, long groupId, Fragment fragment)
		throws Exception {
		...
		if ((approvedFragmentVersion == null) &&
			...
			throw new FragmentEntryVersionsRequiredException();
		}

		if (fragmentEntry.isHead() && (approvedFragmentVersion == null)) {
			throw new FragmentEntryUnpublishException();
		}
		...
		if (approvedFragmentVersion != null) {
			...
			updatedFragmentEntry = _fragmentEntryService.updateFragmentEntry(
				fragmentEntryId, fragmentCollectionId, fragment.getName(), null,
				null, null, GetterUtil.getBoolean(fragment.getCacheable()),
				null, null, fragmentEntry.getPreviewFileEntryId(),
				GetterUtil.getBoolean(fragment.getReadOnly()),
				fragmentEntry.getTypeOptions(), WorkflowConstants.STATUS_DRAFT);
