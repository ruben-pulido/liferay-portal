...
public class ExpressionVisitorImpl implements ExpressionVisitor<Expression> {
	...
	private Expression _getExpression(
		...
		else if((uriResources.size() > 1) && (uriResource instanceof UriResourceComplexProperty)) {

			return new ComplexPropertyExpressionImpl(
				uriResource.getSegmentValue(),
				(PropertyExpression) _getExpression(uriResources.subList(1, uriResources.size())));
		}

		else if((uriResources.size() > 1) && (uriResource instanceof UriResourcePrimitiveProperty)) {
			...
			if(lambdaUriResource instanceof UriResourceLambdaAny) {
				...
				try {
					return new CollectionPropertyExpressionImpl(
						(PropertyExpression)_getExpression(uriResources.subList(0, uriResources.size() - 1)),
		...
		throw new UnsupportedOperationException(
			"UriResource of type: " + uriResource.getClass() +
				" is not supported");
	}
	...
	private PropertyExpression _getPropertyExpression(
		...
		throw new ExpressionVisitException("Unsupported");
	}
