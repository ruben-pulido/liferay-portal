...
public class DefaultMediaObjectNestedCollectionResourceTest
	...
	public void testGetAdaptiveMedias() throws Exception {
		...
		AdaptiveMedia<AMImageProcessor> adaptiveMedia512px =
			adaptiveMediaList.get(0);

		URI contentURL512px = adaptiveMedia512px.getURI();

		byte[] contentBytes512px = IOUtils.toByteArray(
			new URL(TestPropsValues.PORTAL_URL + contentURL512px).openStream());

		assertThat(
			contentBytes512px.length,
			is(both(greaterThan(0)).and(lessThanOrEqualTo(bytes.length))));

		assertThat(
			adaptiveMedia512px.getValueOptional(
